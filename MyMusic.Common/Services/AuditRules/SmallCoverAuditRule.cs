...
public class SmallCoverAuditRule(IOptions<AuditConfig> config) : IAuditRule
{
    public long Id => 6;
    public string Name => "Small Sized Covers";
    public string Icon => "IconPhotoMinus";
    ...
    public async Task<int> Scan(MusicDbContext db, long ownerId, CancellationToken cancellationToken = default)
    {
        ...
        var songsWithSmallCover = await db.Songs
        ...
        var nonConformities = songsWithSmallCover.Select(songId => new AuditNonConformity
        {
            SongId = songId,
            AuditRuleId = Id,
            OwnerId = ownerId,
