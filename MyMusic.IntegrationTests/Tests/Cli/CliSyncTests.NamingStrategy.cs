...
public partial class CliSyncTests
{
    ...
    public async Task Sync_WithCustomNamingTemplate_ShouldPlaceFileInCorrectFolder()
    {
        ...
        await _cli.SetNamingTemplateAsync(namingTemplate);
        ...
        await _cli.CreateSongAsync(songA, pathA);
        ...
        result2.Downloaded.ShouldBe(1);

        var expectedPath = $"2025/{songA.Title} - {songA.Artists[0]}.mp3";
        _cli.FileShouldExist(expectedPath);
    }
    ...
    public async Task Sync_ShouldPreserveOriginalFolderOnMetadataChange()
    {
        ...
        var createdPaths = await _cli.CreateSongsAsync(
        ...
        var newServerSong = songE with { DeviceIds = [_cli.DeviceId] };
        await _serverSongs.SeedAsync(RequestContext, UserId, [newServerSong]);

        // Second sync downloads all three changes: year-changed A, title-changed B, and new song E
        var result2 = await CliRunner.SyncAsync(_cli);
        result2.ShouldBeSuccessful();
        result2.Downloaded.ShouldBe(3, "Should download 3 songs: year-changed A, title-changed B, and new E");

        // Song A changed year but original_folder should be preserved — file stays in the same folder
        _cli.FileShouldExist(createdPaths[0], "Song A (year change) should stay in same folder");

        // Song B was renamed — old path should be gone, new path should appear
        var expectedPathB = $"2025/{updatedTitle} - {songB.Artists[0]}.mp3";
        _cli.FileShouldNotExist(createdPaths[1], "Old path for Song B should be removed");
        _cli.FileShouldExist(expectedPathB, "Song B (title change) should be renamed");

        // Songs C and D were not modified — they should remain unchanged
        _cli.FileShouldExist(createdPaths[2], "Song C should remain unchanged");
        _cli.FileShouldExist(createdPaths[3], "Song D should remain unchanged");
        ...
        _cli.FileShouldExist(expectedPathE, "New server song E should be downloaded with year-based path");
    }
    ...
    public async Task Sync_ShouldPreserveFolderOnYearChange_SingleSong()
    {
        ...
        var createdPathA = await _cli.CreateSongAsync(songA, pathA);

        _cli.FileExists(createdPathA).ShouldBeTrue($"Created path should exist: {createdPathA}");
        ...
        _cli.FileShouldExist(createdPathA, "Path should exist after initial sync");
        ...
        var result2 = await CliRunner.SyncAsync(_cli);
        result2.ShouldBeSuccessful();
        result2.Downloaded.ShouldBe(1, "Should download the year-changed song");

        // File should stay in the original 2024 folder (original_folder preserved)
        _cli.FileShouldExist(createdPathA, "File should remain in original folder after year change");

        // Local file metadata should reflect the new year (2025)
        var fullPath = Path.Combine(_cli.RepositoryPath, createdPathA);
    ...
    public async Task Sync_ShouldDownloadWithYearPath_WhenSongAddedToDeviceAfterServerCreation()
    {
        ...
        var songA = SongsFixture.DefaultSongs[1]; // The Alibi, year 2024
        var songsData = await _serverSongs.SeedAsync(RequestContext, UserId, [songA]);
        ...
        _cli.FileShouldExist(expectedPath, "Song should be downloaded with year-based path");
        await FileValidator.AssertMetadataAsync(_cli.GetSongPath(expectedPath), title: songData.Title);
    }
}
